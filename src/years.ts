const getPositive = (value: number): Number[] => Array.apply(null, Array(value + 1)).map((_: any, i: number) => i);
const getNegative = (value: number): Number[] => Array.apply(null, Array(value + 1)).map((_: any, i: number) => -i);
